...
class ContentService
{
    ...
    public static function getCmsTour($params, $isPaginate = false)
    {
        $query = CmsProduct::selectRaw('tb_tour.* ')
            ->when(!empty($params['keyword']), function ($query) use ($params) {
                $keyword = $params['keyword'];
                return $query->where(function ($where) use ($keyword) {
                    return $where->where('tb_tour.title', 'like', '%' . $keyword . '%');
                });
            })
            ->when(!empty($params['id']), function ($query) use ($params) {
                return $query->where('tb_tour.id', $params['id']);
            })
            ->when(!empty($params['different_id']), function ($query) use ($params) {
                return $query->where('tb_tour.id', '!=', $params['different_id']);
            });

        if (!empty($params['status'])) {
            $query->where('tb_tour.status', $params['status']);
        } else {
            $query->where('tb_tour.status', "!=", Consts::STATUS_DELETE);
        }
        ...
        if (!empty($params['order_by'])) {
            if (is_array($params['order_by'])) {
                foreach ($params['order_by'] as $key => $value) {
                    $query->orderBy('tb_tour.' . $key, $value);
                }
            } else {
                if ($params['order_by'] == 'price') {
                    $query->orderByRaw('tb_tour.price' . $params['order_by'] . ' asc');
                } else {
                    $query->orderByRaw('tb_tour.price' . $params['order_by'] . ' desc');
                }
            }
        } else {
            $query->orderByRaw('tb_tour.iorder ASC, tb_tour.id DESC');
        }
    ...
    public static function getCmsPost($params, $isPaginate = false)
    {
        ...
        if (!empty($params['order_by'])) {
            if (is_array($params['order_by'])) {
                ...
                if ($params['order_by'] == 'price') {
                    $query->orderByRaw('CAST(JSON_VALUE(tb_cms_posts.json_params, "$.price") AS unsigned) DESC');
                } else {
